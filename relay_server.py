...
# Ścisła biała lista znaków dla room_id (Base32)
_SAFE_ID_CHARS = set(
...
def validate_str(value, max_len: int, field: str) -> str | None:
    ...
    Odrzuca: nie-string, pusty, za długi, znaki kontrolne.
    Dla room_id stosuje ścisłą białą listę znaków.
    Dla sender (nicku) pozwala na spacje i polskie znaki, blokując znaki kontrolne (log-injection).
    """
    ...
    if field == "room_id":
        # room_id to skrót Base32, musi zawierać tylko czyste znaki ASCII
        if not all(c in _SAFE_ID_CHARS for c in v):
            return None
    elif field == "sender":
        # Nick może zawierać spacje, polskie znaki i emotki, ale filtrujemy znaki kontrolne (\r, \n, \t)
        for c in v:
